...
export default function AdminCities() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!response.isSuccess) {
        throw new Error(response.message || "ذخیره شهر ناموفق بود.");
      }
      reset();
      await load();
    } catch (err) {
  ...
  const remove = async (id) => {
    if (!window.confirm("آیا از حذف این شهر مطمئن هستید؟")) return;
    try {
      const response = await deleteCity(id);
      if (!response.isSuccess) {
        throw new Error(response.message || "حذف شهر ناموفق بود.");
      }
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : extractErrorMessage(err));
  ...
  return (
    <section>
      <div className="admin-page-header">
        <div>
          <h1>مدیریت شهرها</h1>
          <p>افزودن، ویرایش و حذف شهرهای خبری.</p>
        </div>
      </div>
